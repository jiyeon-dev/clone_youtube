...
- [x] 채널 페이지 - carousel 윈도우 사이즈에 따른 card 수 변경
- [ ] 구독 - 채널 탐색 페이지
...
- 채널 - 라이브 방송 목록 조회시 현재 진행중인 라이브와 종료된 라이브 동시에 검색이 안된다.
  - `search:list` eventType 옵션이 한 개만 선택이 되기 때문에 `completed` 또는 `live` 로 조회 가능하다.
  - completed 로 조회하도록 함.
